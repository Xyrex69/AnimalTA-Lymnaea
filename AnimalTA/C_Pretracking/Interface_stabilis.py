...
import os
import cv2
from AnimalTA.A_General_tools import Class_change_vid_menu, Class_Lecteur, UserMessages, User_help, Class_stabilise, Color_settings
import math
import numpy as np
...
class Stabilise(Frame):
    ...
    def __init__(self, parent, boss, main_frame, Video_file, speed=0, **kwargs):
        ...
        self.Param1=IntVar()
        Scale_pts_dist=Scale(self.canvas_validate, variable=self.Param1, from_=0, to=1000, resolution=10, orient=HORIZONTAL, label=self.Messages["Stab6"], command=self.redo_opt_pts, **Color_settings.My_colors.Scale_Base)
        ...
        self.Param2=IntVar()
        Scale_pts_blck=Scale(self.canvas_validate, variable=self.Param2, from_=1, to=50, resolution=1, orient=HORIZONTAL, label=self.Messages["Stab7"], command=self.redo_opt_pts, **Color_settings.My_colors.Scale_Base)
        ...
        Scale_pts_qua=Scale(self.canvas_validate, variable=self.Param3, from_=0.001, to=0.25, resolution=0.001, orient=HORIZONTAL, label=self.Messages["Stab8"], command=self.redo_opt_pts, **Color_settings.My_colors.Scale_Base)
        ...
        self.Param4=IntVar()
        Scale_pts_corner=Scale(self.canvas_validate, variable=self.Param4, from_=10, to=300, resolution=10, orient=HORIZONTAL, label=self.Messages["Stab9"], command=self.redo_opt_pts, **Color_settings.My_colors.Scale_Base)
        ...
        self.B_Validate_NContinue=Button(self.canvas_validate, text=self.Messages["Validate_NC"], command=lambda: self.End_of_window(follow=True), **Color_settings.My_colors.Button_Base)
    ...
    def on_mousewheel(self, event):
        ...
        if event.delta>0 or (self.tool_size.get()>1 and event.delta<0):
            self.tool_size.set(int(self.tool_size.get()  + (event.delta / 120)))
        self.modif_image()
    ...
    def modif_image(self, img=[], aff=False, **kwargs):
        ...
        new_img=cv2.circle(new_img,self.mouse_pos,self.tool_size.get(),(0,0,0),max(1,int(1.5*self.Vid_Lecteur.ratio)))
    ...
    def mouse_over(self, Pt):
        self.mouse_pos=Pt
        self.modif_image()

    def moved_can(self, Pt, event):
        self.mouse_pos=[int(Pt[0]),int(Pt[1])]
