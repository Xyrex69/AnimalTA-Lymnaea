...
class Details_basics(Frame):
    ...
    def calculate_speed(self,  ind):
        ...
        _coos_sm = _smooth_coos_for_dist(self.Copy_Coos, self.main.Vid)
        Dists = np.sqrt(np.diff(_coos_sm[:, 0]) ** 2 + np.diff(_coos_sm[:, 1]) ** 2) / float(self.main.Vid.Scale[0])
    ...
    def update_vals(self):
        ...
        # Each metric is computed independently and guarded so that a failure
        # (e.g. an all-NaN slice raising a warning-as-error in the packaged
        # build) can never abort the whole update and leave every label stuck
        # at its construction default of "0.0".
        import warnings
        with warnings.catch_warnings(), np.errstate(all="ignore"):
            warnings.simplefilter("ignore")
            for fn, var in ((self.show_Mean_Speed, self.Mean_Speed),
                            (self.show_Prop_lost, self.Prop_lost),
                            (self.show_calculate_dist, self.Dist_total)):
                try:
                    fn()
                except Exception as e:
                    print("update_vals:", fn.__name__, repr(e))
                    var.set("NA")
    ...
    def update_vals_flex(self):
        ...
        with warnings.catch_warnings(), np.errstate(all="ignore"):
            ...
            try:
                ...
                print("update_vals_flex: calculate_state", repr(e))
            for fn, var in ((self.show_Mean_Speed_move, self.Mean_Speed_Move),
                            (self.show_Prop_move, self.Prop_move),
                            (self.show_calculate_dist_move, self.Dist_total_move)):
                try:
                    fn()
                except Exception as e:
                    print("update_vals_flex:", fn.__name__, repr(e))
                    var.set("NA")
